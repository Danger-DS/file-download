...
class info(commands.Cog):
    ...
    @commands.command(aliases = ['server', 'serverlink'])
    async def serverinvite(self, ctx):
        """Provides invite link to the bot's discord server."""
        em = discord.Embed(title = 'Server Invite Link', url = 'https://discord.gg/rnrd5EHjdz', description = 'Click title to join Function\'s official discord server.')
        em.set_thumbnail(url = self.client.user.avatar_url)
        await ctx.channel.send(embed = em)
    ...
    async def botinfo(self, ctx):
        ...
        timeDifference = int(round(currentTime - start_time))
        creationDate = self.client.user.created_at.date().strftime('%m-%d-%Y')
        todayDate = datetime.datetime.today().strftime('%m-%d-%Y')
        print(days(f'{creationDate}', f'{todayDate}'))
        # print((todayDate - creationDate).days)
        ...
        em.add_field(name = 'Current Uptime', value = str(datetime.timedelta(seconds = timeDifference)), inline = True)
        em.add_field(name = 'Number of Commands', value = f'{commandCount}', inline = True)
        em.add_field(name = 'Command Usage Count', value = f'{totalComCount}', inline = True)
        em.add_field(name = 'Age', value = f'{days(creationDate, todayDate)} days young', inline = True)
        em.set_thumbnail(url = self.client.user.avatar_url)
    ...
    @commands.command(aliases = ['ranklist', 'scoreboard', 'ranking', 'rankings', 'lb', 'sb'])
    async def leaderboard(self, ctx):
        """Ranking users across all servers the bot is in (on level)."""
        font_thick = ImageFont.truetype('Montserrat-SemiBold.otf', 60)
        font_top_three = ImageFont.truetype('Montserrat-SemiBold.otf', 45)
        font_thin = ImageFont.truetype('Montserrat-Regular.otf', 25)
        font_seven = ImageFont.truetype('Montserrat-SemiBold.otf', 35)
        ...
        for name in ranking:
            ...
            if len(topTen) > 9:
                ...
                continue
            elif name.split('#')[0] not in topTen:
                topTen.append(name.split('#')[0])
                topScores.append(scores[index])
        colors = ['#FCC404', 'silver', '#CD7F32', '#9300FF', '#4700B3', 'blue', 'lime', '#FAFA37', '#EC6D10', 'red']
        print(topScores)
        print(topTen)

        img = Image.new("RGB", (800, 1000), 'black')
        img = ImageOps.expand(img, border = 15, fill = '#87CEFA')

        img_edit = ImageDraw.Draw(img)
        w, h = img_edit.textsize('Leaderboard - Top 10', font = font_thick)
        img_edit.text(((800-w)/2,35), 'Leaderboard - Top 10', '#CDEEFD', font_thick)
        img_edit.line([((800-w)/2, (40+h)), (((800-w)/2)+w, 40+h)], fill = '#CDEEFD', width = 10)
        startHeight = 150
        # img_edit.ellipse((35, startHeight+5, 85, startHeight+55), fill = '#FCC404')
        for aName in topTen:
            if topTen.index(aName) > 2:
                fontToUse = font_seven
                xLen = 75
            else:
                fontToUse = font_top_three
            if topTen.index(aName) != 9:
                msg = f' #{topTen.index(aName) + 1}   {aName} - {topScores[topTen.index(aName)]} pts'
            else:
                msg = f'#{topTen.index(aName) + 1}  {aName} - {topScores[topTen.index(aName)]} pts'
            if topTen.index(aName) + 1 == 1:
                color = '#FFD700'
            elif topTen.index(aName) + 1 == 2:
                color = 'silver'
            elif topTen.index(aName) + 1 == 3:
                color = '#CD7F32'
            else:
                color = '#E6F6FE'
            wid = 660
            xLen = (800-wid)/2
            img_edit.text((xLen, startHeight), msg, color, fontToUse)
            if topTen.index(aName) > 2:
                startHeight += 77
            else:
                startHeight += 100

        img.save(r'cogs\media\leaderboard.png')
        await ctx.channel.send(file = discord.File('cogs\media\leaderboard.png'))
        print(((800-w)/2), (47+h))

        # em = discord.Embed(title = 'Leaderboard')
        # image = discord.File('cogs\media\leaderboard.png', filename = 'scores.png')
        # em.set_image(url = 'attachment://scores.png')
        # await ctx.channel.send(file = image, embed = em)
